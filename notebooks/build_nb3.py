...
cells.append(mc(
    ...
    "|------|-------------|--------------------------|\n"
    "| 0 | Reinstall + load NB2 results | ~3 min |\n"
    "| 8 | Final: best algo + HPO params × 1M steps | ~37-40 min |\n"
    "| 9 | Curriculum: n_tests=[10,50,200] × 200K each | ~25-30 min |\n"
    "| 10 | Learning curves + reward distribution | ~2 min |\n"
    "| 11 | Coverage histogram + cost efficiency | ~2 min |\n"
    "| 12 | Heatmap + comprehensive summary table | ~2 min |\n"
    "| 13 | ZIP all models + export | ~2 min |\n\n"
    "**Total: ~1.5-2 hours**  (well within Kaggle's 12-hour limit)\n\n"
    "## Timing Reference (from NB1 on this Kaggle session)\n"
    "DQN at 200K steps = 7.4 min → 1M steps ≈ 37 min | Curriculum stages ≈ 7-8 min each"
))
...
cells.append(cc(
    ...
    "fp(f'GPU: {gpu}  |  RL train device: cpu (forced for SB3 MlpPolicy)')\n\n"
    ...
    "fp(f'All results loaded. Best algo: {best_algo}')\n"
    "fp(f'HPO best params: {best_params}')\n"
    "fp(f'HPO best reward: {hpo_result[\"best_value\"]:+.2f}')\n"
    "fp(f'Best baseline:   {best_bl:+.2f} (cost_efficient heuristic)')\n"
    "fp('\\nStage-2 seed summary:')\n"
    "for algo in top2_algos:\n"
    "    rewards = [m['mean_reward'] for m in stage2_results[algo]]\n"
    "    fp(f'  {algo}: mean={np.mean(rewards):+.2f} std={np.std(rewards):.3f}')\n"
))

# Step 8: Final training
cells.append(mc(
    "## Step 8: Final Training — 1M Steps with Best Config\n\n"
    "1 million timesteps with HPO-optimised hyperparameters. "
    "Based on NB1 timing (DQN: 200K = 7.4 min), 1M steps \u2248 **37-40 min**.\n"
    "Progress prints every 50K steps so you know it's running."
))
cells.append(cc(
    ...
    "fp(f'Final Training: {FINAL_STEPS:,} steps | algo={best_algo}')\n"
    "fp(f'HPO params: {best_params}')\n"
    "fp(f'Start: {_time.strftime(\"%H:%M:%S\")}  |  Est. ~37-40 min')\n"
    "fp('=' * 70)\n\n"
    ...
    "fp(f'Training done in {final_train_time/60:.1f}m. Evaluating 200 episodes...')\n"
    "final_metrics = evaluate_trained_model(env, final_model, n_episodes=200)\n"
    "fp(f'\\nFinal Model Results (1M steps, 200 eval episodes):')\n"
    "fp(f'  Mean Reward:  {final_metrics[\"mean_reward\"]:+.2f}')\n"
    "fp(f'  Std  Reward:  {final_metrics[\"std_reward\"]:+.2f}')\n"
    "fp(f'  Accuracy:     {final_metrics[\"accuracy\"]*100:.1f}%')\n"
    "fp(f'  Mean Cost:    {final_metrics.get(\"mean_cost\", 0):.2f}')\n"
    "fp(f'  Mean Tests:   {final_metrics.get(\"mean_tests\", 0):.1f}')\n"
    "fp(f'  Train Time:   {final_train_time/60:.1f} min')\n"
    "fp(f'  vs Baseline:  {final_metrics[\"mean_reward\"] - best_bl:+.2f} reward improvement')\n"
))
...
cells.append(cc(
    ...
    "STAGES = [10, 50, 200]\n\n"
    "fp(f'Curriculum: stages={STAGES}, steps_per_stage={CURR_STEPS:,}')\n"
    "fp(f'Start: {_time.strftime(\\\"%H:%M:%S\\\")}  |  Est. ~7-8 min per stage')\n"
    "fp('=' * 70)\n\n"
    "for n_tests in STAGES:\n"
    "    fp(f'\\n[Stage {STAGES.index(n_tests)+1}/3] n_tests={n_tests}  start={_time.strftime(\\\"%H:%M:%S\\\")}')\n"
    ...
    "    fp(f'  reward={metrics[\"mean_reward\"]:+.2f} | acc={metrics[\"accuracy\"]*100:.1f}% | {elapsed/60:.1f}m | done={_time.strftime(\\\"%H:%M:%S\\\")}')\n\n"
    "fp(f'\\nCurriculum complete – {len(curriculum_results)} stages done')\n"
))
